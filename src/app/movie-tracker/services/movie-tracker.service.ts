...
export class MovieTrackerService {
	...
	protected processSearchMovies(response: HttpResponseBase): Observable<Movie[]> {
		const status = response.status;
		const responseBlob =
			response instanceof HttpResponse ? response.body :
				(<any>response).error instanceof Blob ? (<any>response).error : undefined;
...
export class Movie implements IMovie {
	...
	init(data?: any) {
		if (data) {
			...
			this.watchedOn = data["watchedOn"] ? new Date(data["watchedOn"].toString()) : <any>undefined;
	...
	toJSON(data?: any) {
		...
		data["watchedOn"] = this.watchedOn ? formatDate(this.watchedOn) : <any>undefined;
